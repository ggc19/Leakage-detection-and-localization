function [xx, xx_cost, dxx_cost, dx_cost]=newton_cost2(x1, x2, L, start1, end1, T, M, velocity, nb_sensors)
% Input:  
...
for i=1:2
    ...
    temp2=(-4/velocity)*(M(2, 2)*(T(1,2)-d1(i, 2))+M(2, 3)*(T(1,3)-d1(i, 3))+M(2, 4)*(T(1,4)-d1(i, 4))+M(2, 5)*(T(1,5)-d1(i, 5))+M(2, 6)*(T(1,6)-d1(i, 6))+M(2, 7)*(T(1,7)-d1(i, 7)));         
    cost(i,1)=temp1;
    dx_cost(i,1)=temp2;
end
...
dxx = zeros(1,nb);
dxx(1,1)=-22/velocity; dxx(1,2)=(22+2*xx-L-52)/velocity; dxx(1,3)=(22+2*xx-L-70)/velocity; dxx(1,4)=(22+2*xx-L-110)/velocity; dxx(1,5)=(44+2*xx-L-52)/velocity;
dxx(1,6)=(44+2*xx-L-70)/velocity; dxx(1,7)=(44+2*xx-L-110)/velocity; dxx(1,8)=-18/velocity; dxx(1,9)=-58/velocity; dxx(1,10)=-40/velocity;
xx_cost = zeros(1,1);
dxx_cost = zeros(1,1);
for i=1:1
    temp3=0;
    ...
    temp4=(-4/velocity)*(M(2, 2)*(T(1,2)-dxx(i, 2))+M(2, 3)*(T(1,3)-dxx(i, 3))+M(2, 4)*(T(1,4)-dxx(i, 4))+M(2, 5)*(T(1,5)-dxx(i, 5))+M(2, 6)*(T(1,6)-dxx(i, 6))+M(2, 7)*(T(1,7)-dxx(i, 7)));
    xx_cost(i,1)=temp3;
    dxx_cost(i,1)=temp4;
end
